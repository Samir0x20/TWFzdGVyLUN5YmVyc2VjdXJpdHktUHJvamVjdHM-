...
// @ts-nocheck
    ...
    function displayCreateModal() {
        modalProduct = { name: '', price: 0, category: '', image: null, imagePreview: '', url: ''};
        showCreateModal = true;
    }

    function updateModalProduct() {
        console.log(modalProduct);
        products.updateProduct(modalProduct);
        // Sanitize input
    ...
    function createProduct() {
        if (!modalProduct.name || !modalProduct.category 
            || !modalProduct.price || !modalProduct.image) {
    ...
    function handleFileChange(event) {
        ...
        if (file) {
            ...
            reader.onload = (e) => {
                modalProduct.image = file;
                modalProduct.imagePreview = e.target.result;
            };
    ...
    {#if modalProduct}
        <div class="container m-2">
            ...
            <Label class="space-y-2">
                ...
                <Input type="file" class="text-xl" on:change={handleFileChange} />
            </Label>
        </div>
        <div class="container">
            <img class="card-img-top w-40 h-40 m-auto" src={modalProduct.imagePreview} alt={modalProduct.name} />
        </div>
...
<Modal bind:showModal={showCreateModal} cssClass="text-center">
    ...
    {#if modalProduct}
        <div class="container m-2">
            ...
            <Label class="space-y-2">
                <span>Product image</span>
                <Input type="file" class="text-xl" on:change={handleFileChange} required/>
            </Label>
        </div>
        <div class="container">
            <img class="card-img-top w-40 h-40 m-auto" src={modalProduct.imagePreview} alt={modalProduct.name} />
