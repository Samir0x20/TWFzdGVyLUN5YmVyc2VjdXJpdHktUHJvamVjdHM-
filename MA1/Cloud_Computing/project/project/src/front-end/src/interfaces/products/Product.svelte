<script lang="ts">
  ...
  $: if (showModal) {
    ...
    recommendedProduct = recommendation.getRecommendations(product.name, productInCart.map(item => item.name));
  }
  ...
  <div class="card h-100">
    ...
    <Modal bind:showModal cssClass="text-center">
      ...
      <div class="container">
        ...
        <p> Good choice, eating healthy is like starting fresh.</p>
        <br />
        <hr class="mb-3" />
        <h3>Customers who bought this item also bought</h3>
        {#if recommendedProduct}
          <div class="thumbnail">
            <img class="thumbnail-img" src={recommendedProduct.url} alt={recommendedProduct.name} />
            <div class="thumbnail-details">
              <h2 class="text-2xl">{recommendedProduct.name}</h2>
              <span class="text-xl"><em>{recommendedProduct.price}$</em></span>
            </div>
          </div>
        {:else}
