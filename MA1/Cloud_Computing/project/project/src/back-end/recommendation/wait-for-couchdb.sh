...
until curl -X PUT ${DB_RECOMMENDATION_URL} ; do
  echo "DB (${DB_RECOMMENDATION}) wasn't created - sleeping"
  sleep 1
done
...
cd formatter_output
echo "Creation of views for recommendation DB"
for view in `ls *.js`; do
  curl -X PUT "${DB_RECOMMENDATION_URL}/_design/queries" --upload-file ${view}
  if [[ ${?} != 0 ]]; then
