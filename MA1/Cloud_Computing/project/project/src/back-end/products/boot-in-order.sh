...
if [ "${WITH_PERSISTENT_DATA}" != "" ]; then
  echo "Wait (indefinitely) until the DB creation (name: ${DB_PRODUCTS})."
  echo "The DB URL is: ${DB_PRODUCTS_URL}"
  until response=$(curl --silent --request PUT ${DB_PRODUCTS_URL}) && check_response "$response"; do
    echo -e "\t DB (${DB_PRODUCTS}) wasn't created - trying again later..."
    sleep 2
  done
  echo "DB (${DB_PRODUCTS}) was created!"
fi
