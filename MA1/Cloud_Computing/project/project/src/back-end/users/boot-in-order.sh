...
if [ "${WITH_PERSISTENT_DATA}" != "" ]; then
  ...
  echo "The DB URL is: ${DB_USERS_URL}"
  until response=$(curl --silent --request PUT ${DB_USERS_URL}) && check_response "$response"; do
    echo -e "\t DB (${DB_USERS}) wasn't created - trying again later..."
    sleep 2
  done
  echo "DB (${DB_USERS}) was created!"

  echo "Wait (indefinitely) until the DB creation (name: ${DB_SESSIONS})."
  echo "The DB URL is: ${DB_SESSIONS_URL}"
  until response=$(curl --silent --request PUT ${DB_SESSIONS_URL}) && check_response "$response"; do
    echo -e "\t DB (${DB_SESSIONS}) wasn't created - trying again later..."
    sleep 2
  done
  echo "DB (${DB_SESSIONS}) was created!"
fi
