...
function getWorkersIps {
  rm -f tmp nodes
  sudo docker node ls | awk '{print $1, $2}' > tmp
  ...
  for w in ${workers} ; do
    ip=`sudo docker node inspect ${w} | grep Addr | awk '{print $2}'`
...
stack=`sudo docker stack ls | grep ${1} | awk '{print $1}'`
[ "${stack}" == "" ] && endScript "unknownStack"

service=`sudo docker stack services ${stack} | grep ${stack}_${2}`
...
function updateAvg {
  sum=""
  REPLICAS=0
  id=`sudo docker ps | grep ${service}`
  if [ "${id}" != "" ] ; then
    id=`echo ${id} | awk '{print $1}'`
    data=`sudo docker stats --no-stream ${id}`
    sum=`echo ${data} | awk '{print $19}' | awk -F '%' '{print $1}'`
    let REPLICAS=REPLICAS+1
  fi
  for ip in ${WORKERS_IPS} ; do
    ip=`echo ${ip} | awk -F '"' '{print $2}'`
    id=`ssh -o StrictHostKeyChecking=no ${ip} "sudo docker ps" | grep ${service}`
    if [ "${id}" != "" ] ; then
      id=`echo ${id} | awk '{print $1}'`
      data=`ssh -o StrictHostKeyChecking=no ${ip} "sudo docker stats --no-stream ${id}"`
  ...
  ok=`python3 -c "l = [${sum}] ;r = 1 if len(l) != 0 else 0 ;print(r) "`
  if [ ${ok} -eq 1 ]; then
    AVG=`python3 -c "l = [${sum}] ;print( sum(l) / len(l) ) "`
  else
    AVG="0.0"
  fi
}
...
function tresholdNotReached {
  echo $(python3 -c "r = 1 if ${AVG} < ${TRESHOLD} else 0 ;print(r)")
}
